...
public:
    ...
    NSString *columnName(int64_t col) const noexcept;
    ...
    bool isForRow(int64_t key) const {
        return row.get_key().value == key;
    }
...
private:
    ...
    void setRow(realm::Table const& table, int64_t newRow);
...
RLMObservationInfo *RLMGetObservationInfo(RLMObservationInfo *info, int64_t row, RLMClassInfo& objectSchema);
